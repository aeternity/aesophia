...
%% These helper functions are defined in aeso_ast_infer_types; we forward calls.
pos(T) -> aeso_ast_infer_types:pos(T).
pos(L, C) -> aeso_ast_infer_types:pos(L, C).
pp_loc(T) -> aeso_ast_infer_types:pp_loc(T).
instantiate(T) -> aeso_ast_infer_types:instantiate(T).
typesig_to_fun_t(T) -> aeso_ast_infer_types:typesig_to_fun_t(T).
pp_expr(E) -> aeso_ast_infer_types:pp_expr(E).
pp_expr(Label, E) -> aeso_ast_infer_types:pp_expr(Label, E).

pretty_named_arg({named_arg_t, _Ann, Id, _Type, Default}) ->
    aeso_ast_infer_types:pp_expr("", {typed, [], Id, undefined}) ++ " = " ++ aeso_ast_infer_types:pp_expr("", Default);
pretty_named_arg({var_args, _Ann, _Fun}) ->
    "var_args";
pretty_named_arg(Other) -> pp_expr("", Other).
    ...
    {pos(Fun),
     io_lib:format("when checking the application of\n"
                   ...
                   [pp_typed("", Fun, Inferred),
                    [ ["\n  ", "`" ++ pretty_named_arg(NamedArg) ++ "`"] || NamedArg <- NamedArgs ] ++
                    [ ["\n  ", "`" ++ pp_typed("", Arg, ArgT) ++ "`"]
...
pp_when({checking_init_args, Ann, Con0, ArgTypes0}) ->
    ...
    ArgTypes = instantiate(ArgTypes0),
    {pos(Ann),
     io_lib:format("when checking that the constructor `init` of the contract `~s`"
                   " can be applied to the arguments (~s)",
